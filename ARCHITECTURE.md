...
- The admin console persists rule changes as bounded deltas in `admin.state_file`, without rewriting TOML or rule sources. It also exposes a sanitized effective-config view and may persist only whitelisted immediate overrides (`log_level`, `dns.upstream`, `dns.fallback`); all other fields remain read-only. `--ignore-admin-state` is the startup escape hatch for a bad state file or override.
